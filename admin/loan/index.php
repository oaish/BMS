...
if ($order !== "") {
    $sql = "SELECT Users.UserID, Users.Username, Users.Email, Users.Phone, Loans.LoanId ,Loans.LoanType, Loans.LoanStatus 
            FROM Users
            INNER JOIN Loans ON Users.UserID = Loans.UserId
            ORDER BY $order $type";
} else {
    $sql = "SELECT Users.UserID, Users.Username, Users.Email, Users.Phone, Loans.LoanId, Loans.LoanType, Loans.LoanStatus 
            FROM Users
...
// echo "<script>alert({$users["LoanId"]})</script>";
?>
        ...
        <div class="user-accounts-div">
            <div class="dash-card dash-recent">
                ...
                <div class="user-account-header">
                    ...
                    <div class="user-account-divider"></div>
                    <div class="user-account-card" onclick="setOrder('LoanType')">Loan Type</div>
                    <div class="user-account-divider"></div>
                    <div class="user-account-card" onclick="setOrder('LoanStatus')">Loan Status</div>
                </div>
                <div class="user-accounts">
                    <?php if (empty($users)) { ?>
                        <div class="user-account no-req">
                            <div class="pay-req-acc">No Loan Requests</div>
                        </div>
                    <?php } ?>

                    <?php foreach ($users as $idx => $user) { ?>
                        <?php if($user["LoanStatus"] == "Pending") { ?>
                        <div class="user-account">
                            ...
                            <div style="text-transform: capitalize; color: #fff; position: relative">
                                ...
                                <span style="position: absolute; right: 10px; top: 50%; translate: 0 -50%; cursor: pointer"
                                    onclick="window.location.href = '/BMS/admin/loan/approve.php?id=<?= $user['LoanId'] ?>' "
                                >
